...
import type { AccountState, InvestmentHistoryItem, InvestmentOption } from "../../api/types";
    ...
    return (
        <div className="pageConsole">
            <div className="gridTop">
                <Card title="Welcome">
                    <div>{state?.userName ? `Hi, ${state.userName}!` : "Loading user..."}</div>
                    <div className="muted">Investment console</div>
                </Card>
                                ...
                                <tbody>
                                    {state.activeInvestments.map((inv) => {
                                        const endsInSec = Math.max(
                                            0,
                                            Math.ceil((new Date(inv.endTimeUtc).getTime() - Date.now()) / 1000)
                                        );
                    ...
                    <div className="tableWrap">
                        {options.length ? (
                            <table className="table">
                                ...
                                <tbody>
                                    {options.map((opt) => {
                                        const isActive = !!state?.activeInvestments?.some((a) => a.optionId === opt.id);
                                            ...
                                            <tr className="row" key={opt.id}>
                                                ...
                                                <TableCell>
                                                    <button
                                                        ...
                                                        title={
                                                            isActive ? "Already active" : !canAfford ? "Insufficient balance" : "Invest"
                                                        }
                                                    >
                                                        Invest
                                                    </button>
